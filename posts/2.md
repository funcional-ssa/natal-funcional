...
`let` vai associar um símbolo a um valor ou função. Aqui nós associamos o nome `add` a uma função que recebe dois parâmetros. A sintaxe para funções é bem mais concisa que C#. Parâmetros e argumentos são separados por espaço invés de vírgula, e nós não colocamos parênteses ao redor delas. Os tipos também são inferidos em muitos casos para que você não precise especificá-los quando isso não aperfeiçoa o programa. A última expressão é também retornada pela função automaticamente.
...
Você pode ter visto lambdas (funções sem nome) em várias outras linguagens, e em F# são chamadas `fun`.
...
Currying e closures são os principeis recursos que precisamos para começar a criar funções mais complexas ao combinar mais funções primitivas.
...
`compose` vai rodar primeiro a primeira função, e então usar o resultado dela como argumento para a segunda função. E isso pode ser aninhado para que póssamos funções complexas quaisquer `compose (compose first next) last`.
